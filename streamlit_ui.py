...
section[data-testid="stSidebar"] .stRadio label { color: #a7f3d0 !important; font-size: 0.9rem; }
...
.hero {
  ...
  border-radius: 20px; padding: 70px 60px; color: white;
...
.badge {
  ...
  border-radius:30px; padding:6px 16px; font-size:0.85rem; color:#ccfbf1;
}

/* ── Stat cards (landing) ── */
.stats-row { display:flex; gap:16px; margin-bottom:2rem; flex-wrap:wrap; }
.stat-card {
  flex:1; min-width:150px; background:white; border-radius:16px;
  padding:24px 20px; text-align:center;
  box-shadow:0 4px 20px rgba(0,0,0,0.07); border-top:4px solid var(--teal);
  transition:0.3s;
}
...
.dstat-row { display:flex; gap:16px; margin-bottom:2rem; flex-wrap:wrap; }
.dstat-card {
...
.dstat-number { font-family:'Playfair Display',serif; font-size:2.4rem; font-weight:700; color:#14b8a6; }
.dstat-label  { font-size:.85rem; color:#a7f3d0; margin-top:4px; }
...
.service-card {
  flex:1; min-width:170px; background:white; border-radius:14px;
...
.info-card {
  ...
  border-radius:14px; padding:24px; border:1px solid #a7f3d0;
}
...
.data-table tr:hover td { background:#134e4a; transition:0.3s; }
...
.tag {
  ...
  border-radius:20px; padding:3px 12px; font-size:.8rem; font-weight:500; margin:2px;
}
...
.footer {
  ...
  padding:30px 40px; text-align:center; margin-top:3rem; font-size:.85rem; line-height:2;
}
...
def show_login_page():
    ...
    <div style='text-align:center;padding:10px 0 6px;'>
      ...
      <h2 style='font-family:Playfair Display,serif;color:white;margin:0;'>Staff Portal Login</h2>
...
def show_landing():
    ...
    <div class="hero">
      ...
      <div class="hero-badges">
        <span class="badge">🏅 ISO Certified</span><span class="badge">🕐 24/7 Emergency</span>
        <span class="badge">👨‍⚕️ 50+ Specialists</span><span class="badge">🏥 300 Bed Capacity</span>
      </div>
    </div>
    <div class="stats-row">
    ...
    with c1:
        ...
        <div class="dash-card">
          ...
          <div style="margin-top:12px;">
            <span class="tag">🎯 Patient-First</span><span class="tag">🔬 Latest Technology</span>
            <span class="tag">❤️ Compassionate Care</span><span class="tag">🌿 Holistic Wellness</span>
          </div>
        </div>
        """, unsafe_allow_html=True)
    with c2:
        st.markdown("""
        <div style="background:linear-gradient(135deg,#0f172a,#134e4a);border-radius:14px;padding:28px;color:white;">
...
def ui_delete_doctor():
    ...
    options = {f"ID {d['id']} — {d['name']}": d['id'] for d in st.session_state.doctor_list}
    sel_label = st.selectbox("Select Doctor", list(options.keys()))
    sel_id = options[sel_label]
    if st.button("Delete Doctor", type="primary"):
        st.session_state.doctor_list = [d for d in st.session_state.doctor_list if d['id'] != sel_id]
...
def ui_search_patient():
    ...
    if method == "Name":
        ...
        if q:
            ...
            if not found: st.error("Patient not found.")
    else:
    ...
    with st.form("gen_bill"):
        ...
        if st.form_submit_button("Generate Bill", type="primary"):
            if name:
                ...
                <div class="slip-box">
                  <h3>🧾 Bill #{b['id']}</h3>
                  <p>Patient: <b>{name}</b><br>
                  Doctor Fee: ₹{doc_f} &nbsp;|&nbsp; Room: ₹{room} &nbsp;|&nbsp; Medicine: ₹{med}<br>
                  <span style="font-size:1.3rem;font-weight:700;color:#14b8a6;">Total: ₹{total:,}</span></p>
...
def ui_view_bills():
    ...
    if not bl: st.info("No bills generated yet."); return
    ...
    with st.form("add_sal"):
        ...
        if st.form_submit_button("Generate Salary Slip", type="primary"):
            if emp:
                ...
                st.markdown(f"""
                <div class="slip-box">
                  <h3>💵 Salary Slip — {emp}</h3>
                  <p>Type: {etype} &nbsp;|&nbsp; Month: {month} {year}<br>
                  Basic: ₹{basic:,} &nbsp;|&nbsp; HRA: ₹{hra_a:,.0f} &nbsp;|&nbsp; DA: ₹{da_a:,.0f}<br>
                  Gross: ₹{gross:,.0f} &nbsp;|&nbsp; PF Deduction: ₹{pf_a:,.0f}<br>
                  <span style="font-size:1.3rem;font-weight:700;color:#14b8a6;">Net Salary: ₹{net:,.0f}</span></p>
                </div>""", unsafe_allow_html=True)
...
def ui_delete_salary():
    ...
    options = {f"ID {s['id']} — {s['name']} ({s['month']} {s['year']})": s['id'] for s in st.session_state.salary_list}
    sel_label = st.selectbox("Select Record", list(options.keys()))
    sel_id = options[sel_label]
    if st.button("Delete Record", type="primary"):
        st.session_state.salary_list = [s for s in st.session_state.salary_list if s['id'] != sel_id]
        st.success("Deleted.")
        st.rerun()
...
def admin_dashboard():
    ...
    # ── Sidebar navigation using session state (no duplicate key issues) ──
    with st.sidebar:
        st.markdown("<h2 style='color:#99f6e4;text-align:center;'>🛡️ Admin Panel</h2><hr>", unsafe_allow_html=True)

        # Each section with unique labels — FIX: all labels are globally unique
        nav_options = [
            "🏠 Home",
            "── DOCTORS ──",
            ...
            "── APPOINTMENTS ──",
            "📅 Book Appointment", "📋 View Appointments", "📆 Search by Date",
            "── BILLING ──",
        ...
        # Separator items (not clickable pages)
        separators = {"── DOCTORS ──", "── PATIENTS ──", "── APPOINTMENTS ──", "── BILLING ──", "── SALARY ──"}

        selected = st.radio(
            "Navigate",
            nav_options,
            index=nav_options.index(st.session_state.admin_page) if st.session_state.admin_page in nav_options else 0,
            label_visibility="collapsed"
        )

        # If user clicked a separator, ignore it
        if selected not in separators:
    ...
    menu = st.session_state.admin_page
    ...
    if menu == "🏠 Home":
        ...
        <div class="dash-card">
          ...
          <p>Use the sidebar navigation to manage doctors, patients, appointments, billing, and salary records.</p>
        </div>
        """, unsafe_allow_html=True)
...
def doctor_dashboard():
    ...
    with st.sidebar:
        st.markdown("<h2 style='color:#99f6e4;text-align:center;'>👨‍⚕️ Doctor Panel</h2><hr>", unsafe_allow_html=True)

        # Unique labels for doctor menu
        nav_options = [
            "🏠 Dashboard",
            "👁️ View Patients",
            "🔍 Search Patient",
        ...
        selected = st.radio(
            "Navigate",
            nav_options,
            index=nav_options.index(st.session_state.doctor_page) if st.session_state.doctor_page in nav_options else 0,
            label_visibility="collapsed"
        )
        st.session_state.doctor_page = selected
    ...
    menu = st.session_state.doctor_page
    dispatch = {
        "👁️ View Patients":    ui_view_patients,
        "🔍 Search Patient":    ui_search_patient,
        "📅 View Appointments": ui_view_appointments,
    ...
    if menu == "🏠 Dashboard":
        ...
        <div class="dash-card">
          ...
          <p>Use the sidebar to view patients, check appointments, and search records.</p>
        </div>
        """, unsafe_allow_html=True)
    elif menu in dispatch:
        dispatch[menu]()
...
def reception_dashboard():
    ...
    with st.sidebar:
        st.markdown("<h2 style='color:#99f6e4;text-align:center;'>🗂️ Reception Panel</h2><hr>", unsafe_allow_html=True)
        ...
        selected = st.radio(
            "Navigate",
            nav_options,
            index=nav_options.index(st.session_state.reception_page) if st.session_state.reception_page in nav_options else 0,
            label_visibility="collapsed"
        )
        st.session_state.reception_page = selected
    ...
    menu = st.session_state.reception_page
    dispatch = {
        "➕ Add Patient":      ui_add_patient,
        "👁️ View Patients":   ui_view_patients,
        "🔍 Search Patient":   ui_search_patient,
    ...
    if menu == "🏠 Dashboard":
        st.markdown("""
        <div class="dash-card">
          <h4>Welcome, Receptionist 🗂️</h4>
          <p>Use the sidebar to register patients, book appointments, and generate bills.</p>
        </div>
        """, unsafe_allow_html=True)
    elif menu in dispatch:
        dispatch[menu]()
